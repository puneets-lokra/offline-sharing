...
function buildCandidates(): string[] {
  ...
  // Explicit bridge URL passed as ?bridge=https://...
  ...
  if (typeof window !== 'undefined') {
    ...
    // Don't add GitHub Pages origin — it's not the bridge
...
async function resolveBridgeUrl(): Promise<string | null> {
  ...
  if (resolvedBridgeUrl) {
    ...
    if (still) return resolvedBridgeUrl;
 ...
 */
export async function sendRecord(record: PatientRecord): Promise<boolean> {
 ...
 */
export async function flushRecords(records: PatientRecord[]): Promise<string[]> {
  const synced: string[] = [];
  for (const record of records) {
    const ok = await sendRecord(record);
    if (ok) synced.push(record.id);
  }
  return synced;
}
