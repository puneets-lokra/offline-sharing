...
export interface RecordFormDefaults {
  doctorId?: string;
  doctorName?: string;
}
 ...
 */
export function RecordForm(onSaved: () => void, defaults: RecordFormDefaults = {}): HTMLElement {
  ...
  form.innerHTML = `
    ...
    <p class="form-note">${doctorLocked
      ...
      : 'Record is saved locally and will sync when bridge is reachable.'
    }</p>
  `;
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (doctorLocked) {
        const hiddenInput = form.querySelector<HTMLInputElement>('input[name="doctorId"][type="hidden"]');
