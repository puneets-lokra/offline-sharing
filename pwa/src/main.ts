...
function resolveBridgeUrl(): string {
  if (explicitBridge) return explicitBridge.replace(/\/$/, '');
...
async function fetchRecords(patientId: string): Promise<any[]> {
  try {
...
function renderRecords(container: HTMLElement, records: any[]): void {
  if (records.length === 0) {
