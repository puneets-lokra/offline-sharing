 ...
 */
async function fetchSessionContext(bridgeUrl: string, code: string): Promise<{ doctorId: string; doctorName: string }> {
  if (!code) return { doctorId: '', doctorName: '' };
  try {
...
async function renderRecordList(container: HTMLElement): Promise<void> {
...
async function bleSendPending(listSection: HTMLElement): Promise<{ sent: number; failed: number }> {
...
async function main() {
  ...
  const bridgeCandidates = (() => {
    ...
    if (explicit) c.push(explicit.replace(/\/$/, ''));
    const { protocol, hostname, port } = window.location;
    const origin = `${protocol}//${hostname}${port ? ':' + port : ''}`;
    if (!origin.includes('github.io')) c.push(origin);
  ...
  for (const url of bridgeCandidates) {
    try {
      ...
      if (r.ok) { resolvedBridge = url; break; }
    } catch { /* try next */ }
  }
  ...
  if (wifiQrEl) app.appendChild(wifiQrEl);
  ...
  if (BLESync.isSupported()) {
    ...
    const bleBtn = bleBar.querySelector<HTMLButtonElement>('#ble-btn')!;
    const bleStatusEl = bleBar.querySelector<HTMLSpanElement>('#ble-status')!;
    ...
    bleBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const listSection = document.querySelector<HTMLElement>('.section:last-of-type')!;
  ...
  async function syncLoop() {
    ...
    if (bleSync.status === 'sending') return;
